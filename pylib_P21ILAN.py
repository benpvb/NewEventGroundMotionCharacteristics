...
class Phung2021h_ILAN:
    def __init__(self):
        ...
        self.c1 = np.array([-1.1955, -1.1883, -1.1230, -1.0214, -0.9313, -0.8114, -0.5317, -0.3454, -0.2730, -0.2061, -0.2189, -0.2471, -0.4252, -0.6520, -0.8786, -1.0796, -1.5363, -2.0303, -2.6831, -3.0446, -3.5510, -3.7084, -3.8368, -4.2318, -4.4423])
        ...
        self.phi1a = np.array([-0.3001, -0.3001,-0.2814,-0.2413,-0.2202,-0.1848,-0.1281,-0.1243,-0.1303,-0.1406,-0.1654,-0.1818,-0.2735,-0.4149,-0.4900,-0.5130,-0.5770,-0.7141,-0.8588,-0.8406,-0.8188,-0.7478,-0.6589,-0.5624,-0.4882])
        self.phi1b = np.array([0.0065, 0.0065,0.0298,0.0535,0.0355,0.0331,0.0666,0.0255,0.0375,0.0926,0.0734,0.0988,-0.0187,-0.2005,-0.1777,-0.1864,-0.1201,-0.1202,-0.2858,-0.2577,-0.2158,-0.1865,-0.1399,-0.1255,-0.1019])

        self.phi5a = np.array([0.0529,0.0519,0.0505,0.0463,0.0445,0.0397,0.0402,0.0470,0.0500,0.0325,0.0263,0.0334,0.0495,0.0728,0.0363,0.0481,0.0596,0.0627,0.1243,0.1213,0.2082,0.2371,0.2264,0.1829,0.1287])
        self.phi5b = np.array([0.1230,0.1202,0.1315,0.1391,0.1959,0.2017,0.1607,0.1090,0.0569,0.1167,0.0811,0.0605,0.0098,-0.0205,0.1631,0.3125,0.6821,0.5844,0.4024,0.2898,0.1638,0.1057,0.0993,0.0538,-0.0433])
    ...
    def Ph2021_ILAN_all_per(self, mag, ztor, rake, delta, Rrup, Vs30, Z1):
        ...
        # linear term
        if Vs30 <= 360:
            term14 = self.phi1a*min(np.log(Vs30/1130),0)
        else:
            term14 = self.phi1b*min(np.log(Vs30/1130),0) 
        ...
        if Z1 == -999:
            ...
            del_Z1 = Z1 - Ez_1
            term16 = phi5*(1 - np.exp(-del_Z1/300))
    ...
    def Ph2021_ILAN_p(self, periods, mag, ztor, rake, delta, Rrup, Vs30, Z1):
        ...
        per_all, Sa_all, sigT_all, sigS_all = self.Ph2021_ILAN_all_per(mag, ztor, rake, delta, Rrup, Vs30, Z1)
